...
| **D2** | ~~Kill the `ven-TREEK` pronunciation respelling.~~ **❌ WITHDRAWN Jul 29 — the reading was wrong.** Justin's Jul 25 brief writes final footer copy that *keeps* it: *"Ventriq (ven-TREEK) equips entrepreneurs with…"*. Deliberate written copy beats an inference from a scrambled transcript. **`ven-TREEK` stays.** What he actually asked to kill at 21:23 is unknown — the audio is too garbled to recover, and it was flagged as unconfirmed at the time. | — | **no change** | 21:23–21:51 | ❌ withdrawn |
