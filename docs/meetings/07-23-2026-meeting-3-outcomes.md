...
| **D2** | **Kill the `ven-TREEK` pronunciation respelling.** ✅ **CONFIRMED by Derrick Jul 29** — he was in the room; the garbled transcript read correctly. ⚠️ Justin's Jul 25 brief §15 writes replacement footer copy that still contains *"(ven-TREEK)"* — **that is his error, not a reversal.** Strip it from our copy anyway, and flag it to him so his brief and email templates get corrected too. *(Briefly withdrawn Jul 29 on the brief's evidence, then reinstated on Derrick's direct knowledge.)* | `ven-TREEK` in eyebrow, footer, About, all copy decks | removed | 21:23–21:51 | 🔴 not started |
